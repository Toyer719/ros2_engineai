...
# THEORIQUE du PNG genere, pas une mesure d'impression reelle (rien n'est
# imprime en sim) -- utiliser --marker-size (meme valeur que MARKER_SIZE
# ci-dessous) avec aruco_carton_test.py/carton_pose_publisher.py.
# 0.1016 -> 0.17 (21/08) : au spawn d'origine (x=0, ~2.2m du carton),
# solvePnP ne detectait RIEN du tout (0 detection en 9s, verifie en sim) --
# le marqueur de 10.16cm est trop petit a cette distance en 640x480.
# 0.17m est la plus grande taille qui tient encore sur la face avant du
# carton (largeur Y reelle 0.191m, voir carton_box size dans
# pm01_edu_carton.xml -- 0.17 laisse ~1cm de marge de chaque cote).
...
CARTON_POS = (1.3274, 0.0, 0.681)  # voir <body name="carton" pos="..."> dans pm01_edu_carton.xml (test2)
...
# 2026-08-26 : CARTON_POS resynchronise avec le carton "test2" (podium
# 91x53.5x71cm eloigne de 4 pas, voir tools/robot_arm_ik_podium_test2/ et
# assets/resource/pm01_edu_carton.xml) -- etait reste a l'ancienne position
# (2.2, 0, 0.796, table 72cm) apres la mise a jour de la scene, ce qui faisait
# projeter le marqueur composite (_composite_marker) a une profondeur fausse
# -> homographie degeneree -> image publiee entierement noire (mean=0),
# donc 0 detection ArUco quelle que soit la distance reelle du robot.
