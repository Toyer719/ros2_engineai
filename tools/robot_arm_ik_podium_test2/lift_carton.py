...
PODIUM_XY = (1.6449, 0.0)              # 2026-08-26 (test2, corrige) : PAS calcule via une distance
                                        # bassin absolue (ancien calcul faux -- utilisait X=-0.075145
                                        # comme position robot home, releve dans docs/pm01_box_
                                        # geometry.txt, alors que dans CE modele le robot demarre
                                        # reellement a X=0.0 -- ecart de 7.5cm qui laissait une marge
                                        # corps-podium trop faible, robot bloque contre le podium).
                                        # Repris directement de l'ecart relatif deja valide du test1
                                        # (carton a 0.3674 du robot home), decale de 0.96m (4 pas) --
                                        # garantit la MEME marge corps-podium (12.5cm) et la meme
                                        # portee bras (pinch_x=0.30) qu'en test1 apres la marche.
...
CARTON_XY = (1.3274, 0.0)

# 2026-08-26 (test2, corrige) : carton = position home du robot (X=0.0, VERIFIE
# directement dans ce modele, pas la valeur perimee -0.075145 de docs/pm01_box_
# geometry.txt) + le meme ecart que le test1 (0.3674) + 0.96m de marche (4 pas).
# Taille du carton INCHANGEE. A cette distance sans marcher, le bras seul n'y
# arrive pas -- il faut marcher() avant de viser (voir main()).
...
GAINS = {
    ...
    "WAIST_YAW": (150.0, 3.0), "SHOULDER_PITCH": (250.0, 5.0), "SHOULDER_ROLL": (250.0, 5.0),
    "SHOULDER_YAW": (250.0, 5.0), "ELBOW_PITCH": (250.0, 5.0), "ELBOW_YAW": (250.0, 5.0),
    "HEAD_YAW": (30.0, 1.0),
}
# Bras (SHOULDER_*/ELBOW_*) remontes de 60-80 a 250 le 2026-08-26 -- l'ancien gain,
# pense pour un mouvement libre/doux, etait trop faible pour vraiment porter le carton
# (contact detecte mais objet immobile, verifie directement via data.ncon/data.xpos --
# meme avec un serrage tres agressif). 250 = meme ordre que Lever/lift.py (ROS), qui a
# ete valide fonctionnel le 2026-08-26 matin.
...
class PM01LiftSim:
    ...
    def soulever(self, pinch_x=0.30, pinch_y=0.22, pinch_z=-0.1392, squeeze_y=0.1405, lift_z=0.146,
...
def main():
    ...
    parser.add_argument("--pas-avant", type=float, default=0.96,
                         help="Distance de marche (m, marcher() -- vraie animation de pas cette "
                              "fois, pas une teleportation) avant de viser -- 0.96m = 4 pas robot "
                              "a la vitesse/periode par defaut de marcher() (0.4m/s x 0.6s x 4), "
                              "ramene le torse a la meme distance carton que le test1 valide "
                              "(29cm). 0 pour desactiver.")
    parser.add_argument("--pinch-y", type=float, default=0.22,
    ...
    if args.pas_avant:
        n_pas_estime = round(args.pas_avant / (0.4 * STEP_PERIOD))
        print(f"[INFO] Marche de {args.pas_avant}m (~{n_pas_estime} pas)...", flush=True)
        sim.marcher(x=args.pas_avant)
