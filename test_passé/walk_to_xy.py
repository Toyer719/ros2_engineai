...
class SimStateListener:
    ...
    def height(self):
        """z courant (m), ou None si aucun message recu -- utilise pour
        verifier que le robot est reellement debout (voir memoire
        project_choreo_native_carton : un sleep fixe apres gp.stand() peut
        tomber sur un robot deja effondre, que pd_stand ne recupere pas
        toujours)."""
        with self._lock:
            state = self._latest
        if state is None:
            return None
        return state.base_link_position[2]
